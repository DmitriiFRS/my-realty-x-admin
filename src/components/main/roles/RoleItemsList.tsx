...
import { IRolesResponse } from '@/types/roles.type';
import React, { useEffect } from 'react';

export default function RoleItemsList() {
   const [list, setList] = React.useState<IRolesResponse | null>(null);
   useEffect(() => {
      const fetchData = async () => {
         const result = await entitiesService.getEntity(`roles/all`);
         console.log(result);
         setList(result);
      };
      fetchData();
   ...
   const handleDelete = async (id: number) => {
      ...
      const confirmed = window.confirm('Вы действительно хотите удалить роль?');
      if (!confirmed) {
         return;
      }
         ...
         <div className="p-6 min-h-screen bg-slate-50">
            <div className="max-w-6xl mx-auto">
               <div className="flex items-center justify-between mb-6">
                  <h1 className="text-2xl font-semibold text-slate-800">Список ролей</h1>
               </div>
                  ...
                  <table className="w-full table-fixed text-sm">
                     <thead className="bg-gray-50">
                        <tr className="text-xs text-gray-500">
                           ...
                           <th className="p-3 text-left w-40">slug</th>
                     ...
                     <tbody>
                        {list.data.map((el) => (
                           <tr key={el.id} className="border-t hover:bg-gray-50">
                              ...
                              <td className="p-3 text-slate-700">{el.slug}</td>
